...
namespace :unicorn do
  ...
  desc 'Unicorn停止(QUIT)'
  task(stop: :environment) { process_kill(:QUIT) }

  desc 'Unicorn再起動(HUP)'
  task(restart: :environment) { process_kill(:HUP) }

  desc 'Unicorn緩やかな再起動(USR2)'
  task(graceful: :environment) { process_kill(:USR2) }

  desc 'Unicornのワーカープロセスを増やす(TTIN)'
  task(increment: :environment) { process_kill(:TTIN) }

  desc 'Unicornのワーカープロセスを減らす(TTOU)'
  task(decrement: :environment) { process_kill(:TTOU) }
