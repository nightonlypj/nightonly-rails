...
RSpec.describe :task_event, type: :task do
  ...
  describe 'task_event:create_send_notice' do
    ...
    let_it_be(:tasks) do
      ...
      [
        FactoryBot.create(:task, :skip_validate, :high, space: space, started_date: Date.new(2023, 1, 3), ended_date: Date.new(2023, 1, 4), created_user: user),
        FactoryBot.create(:task, :skip_validate, :middle, space: space, started_date: Date.new(2022, 12, 30), ended_date: Date.new(2023, 1, 3), created_user: user),
        FactoryBot.create(:task, :skip_validate, :low, space: space, started_date: Date.new(2023, 1, 3), ended_date: Date.new(2023, 1, 4), created_user: user),
        FactoryBot.create(:task, :skip_validate, :none, space: space, started_date: started_date, ended_date: Date.new(2023, 1, 5), created_user: user)
      ]
    end
    before_all do # NOTE: 対象外
      task = FactoryBot.create(:task, :skip_validate, :no_end, space: space, started_date: Date.new(2023, 1, 4), created_user: user)
      FactoryBot.create(:task_cycle, :weekly, task: task, wday: :mon, handling_holiday: :after, period: 2) # 12/30-1/3（月曜＋翌営業日）
      task = FactoryBot.create(:task, :skip_validate, :no_end, space: space, started_date: Date.new(2022, 12, 31), created_user: user)
      FactoryBot.create(:task_cycle, :monthly, :day, task: task, day: 2, handling_holiday: :before, period: 1) # 12/30（前営業日）
      task = FactoryBot.create(:task, :skip_validate, :no_end, space: space, started_date: Date.new(2023, 1, 4), created_user: user)
      FactoryBot.create(:task_cycle, :yearly, :business_day, task: task, month: 1, business_day: 1, period: 2) # 12/30-1/3（第1営業日）
      task = FactoryBot.create(:task, :skip_validate, :no_end, space: space, started_date: Date.new(2023, 1, 5), created_user: user)
      FactoryBot.create(:task_cycle, :yearly, :week, task: task, month: 1, week: :first, wday: :wed, handling_holiday: :after, period: 3) # 12/30-1/4（第1水曜）
    end
    let(:current_task_events) { TaskEvent.where(space: space).order(:id) }
    let(:current_send_histories) { SendHistory.where(space: space).order(:id) }
    ...
    shared_context 'タスク周期作成' do |create = { today: false, next: false }|
      ...
      let_it_be(:except_today_task_events) do
        ...
        [
          { task_cycle_id: today_task_cycles[0].id, started_date: started_date, ended_date: Date.new(2023, 1, 3) },
          { task_cycle_id: today_task_cycles[1].id, started_date: started_date, ended_date: Date.new(2022, 12, 30) },
          { task_cycle_id: today_task_cycles[2].id, started_date: started_date, ended_date: Date.new(2023, 1, 3) },
          { task_cycle_id: today_task_cycles[3].id, started_date: started_date, ended_date: Date.new(2023, 1, 4) }
        ]
      end
      ...
      let_it_be(:except_next_task_events) do
        ...
        [
          { task_cycle_id: next_task_cycles[0].id, started_date: started_date, ended_date: Date.new(2023, 1, 4) },
          { task_cycle_id: next_task_cycles[1].id, started_date: started_date, ended_date: Date.new(2023, 1, 3) },
          { task_cycle_id: next_task_cycles[2].id, started_date: started_date, ended_date: Date.new(2023, 1, 4) },
          { task_cycle_id: next_task_cycles[3].id, started_date: started_date, ended_date: Date.new(2023, 1, 5) }
        ]
      end
    end
    ...
    shared_context 'タスクイベント作成' do |create = { expired: false, today: false, next: false }|
      let_it_be(:expired_task_events) do
        ...
        [FactoryBot.create(:task_event, :waiting_premise, space: space, started_date: target_date - 1.day, ended_date: target_date - 1.day)] # 12/29
      end
    ...
    shared_examples_for '通知履歴作成' do |status|
      let_it_be(:send_histories) do
        [
          FactoryBot.create(:send_history, :start, :slack, status, send_setting: send_setting, target_date: target_date),
          FactoryBot.create(:send_history, :start, :email, status, send_setting: send_setting, target_date: target_date)
        ]
      end
    end
    ...
    shared_examples_for '[当日（営業日）][(翌営業日・終了確認)開始時間より後][ある][*][通知済み/エラー]翌営業日・終了確認' do |status|
      let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_completed: next_notice_completed) }
      ...
      context '通知済み' do
        let_it_be(:send_histories) do
          send_histories_start + [
            FactoryBot.create(:send_history, :next, :slack, :success, send_setting: send_setting, target_date: target_date),
            FactoryBot.create(:send_history, :next, :email, :success, send_setting: send_setting, target_date: target_date)
          ]
        end
      ...
      context '通知エラー' do
        let_it_be(:send_histories) do
          send_histories_start + [
            FactoryBot.create(:send_history, :next, :slack, :failure, send_setting: send_setting, target_date: target_date),
            FactoryBot.create(:send_history, :next, :email, :failure, send_setting: send_setting, target_date: target_date)
          ]
        end
        it_behaves_like 'OK', :next, { history: true, send: true }
      end
    ...
    shared_examples_for '[当日（営業日）][(開始確認)開始時間より後][ある][ある][*]開始確認' do
      let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, start_notice_completed: start_notice_completed) }
      context '未通知' do
        let(:send_histories) { [] }
    ...
    context '対象日が前日（営業日）' do # 手動実行
      ...
      context 'タスク周期がある' do
        ...
        context '通知設定がある' do
          let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space) }
      ...
      context 'タスク周期がない' do
        ...
        context '通知設定がある（必ず通知する）' do
          let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: true) }
          it_behaves_like 'OK', :next, { history: true, send: true }, { dry_run: false, send_notice: true }
          it_behaves_like 'OK', :next, { history: true, send: false }, { dry_run: false, send_notice: false }
          it_behaves_like 'OK', :next, { history: false, send: false }, { dry_run: true, send_notice: true }
        end
        context '通知設定がある（必ずしも通知しない）' do
          let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: false) }
      ...
      context '実行時間が(開始確認)開始時間より後' do
        ...
        context 'タスク周期がない' do
          ...
          context '通知設定がある（必ず通知する）' do
            let(:current_time) { current_date + send_setting.start_notice_start_hour.hours }
            context '完了通知する' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, start_notice_required: true, start_notice_completed: true) }
              it_behaves_like 'OK', :start, { history: true, send: true }
            end
            context '完了通知しない' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, start_notice_required: true, start_notice_completed: false) }
              it_behaves_like 'OK', :start, { history: true, send: true }
            end
          end
          context '通知設定がある（必ずしも通知しない）' do
            let(:current_time) { current_date + send_setting.start_notice_start_hour.hours }
            context '完了通知する' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, start_notice_required: false, start_notice_completed: true) }
              it_behaves_like 'OK', :start, { history: true, send: false }
            end
            context '完了通知しない' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, start_notice_required: false, start_notice_completed: false) }
              it_behaves_like 'OK', :start, { history: true, send: false }
            end
          end
        end
      ...
      context '実行時間が(翌営業日・終了確認)開始時間より後' do
        ...
        context 'タスク周期がない' do
          ...
          context '通知設定がある（必ず通知する）' do
            let(:current_time) { current_date + send_setting.next_notice_start_hour.hours }
            context '完了通知する' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: true, next_notice_completed: true) }
              it_behaves_like 'OK', :next, { history: true, send: true }
            end
            context '完了通知しない' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: true, next_notice_completed: false) }
              it_behaves_like 'OK', :next, { history: true, send: true }
            end
          end
          context '通知設定がある（必ずしも通知しない）' do
            let(:current_time) { current_date + send_setting.next_notice_start_hour.hours }
            context '完了通知する' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: false, next_notice_completed: true) }
              it_behaves_like 'OK', :next, { history: true, send: false }
            end
            context '完了通知しない' do
              let_it_be(:send_setting) { FactoryBot.create(:send_setting, :changed, :slack, :email, space: space, next_notice_required: false, next_notice_completed: false) }
              it_behaves_like 'OK', :next, { history: true, send: false }
            end
          end
        end
