...
class NoticeMailerPreview < ActionMailer::Preview
  ...
  def incomplete_task_start(notice_target = :start)
    ...
    send_setting = FactoryBot.build_stubbed(:send_setting, :email, space:)
    send_history = FactoryBot.build_stubbed(:send_history, notice_target, send_setting:)
  ...
  def next_task_events(notice_target, space)
    ...
    task = FactoryBot.build_stubbed(:task, :high, space:)
    task_cycle = FactoryBot.build_stubbed(:task_cycle, task:)
    FactoryBot.build_stubbed_list(:task_event, count, :tommorow_start, task_cycle:)
  end
  ...
  def expired_task_events(space, assigned_user)
    ...
    task = FactoryBot.build_stubbed(:task, :middle, space:)
    task_cycle = FactoryBot.build_stubbed(:task_cycle, task:)
    FactoryBot.build_stubbed_list(:task_event, count, :yesterday_end, :waiting_premise, :assigned, assigned_user:, task_cycle:)
  end
  ...
  def end_today_task_events(space, assigned_user)
    ...
    task = FactoryBot.build_stubbed(:task, :low, space:)
    task_cycle = FactoryBot.build_stubbed(:task_cycle, task:)
    FactoryBot.build_stubbed_list(:task_event, count, :today_end, :processing, :assigned, assigned_user:, task_cycle:)
  end
  ...
  def date_include_task_events(space)
    ...
    task = FactoryBot.build_stubbed(:task, :none, space:)
    task_cycle = FactoryBot.build_stubbed(:task_cycle, task:)
    FactoryBot.build_stubbed_list(:task_event, count, :update_end, task_cycle:)
  end
  ...
  def completed_task_events(space, assigned_user)
    ...
    task_cycle = FactoryBot.build_stubbed(:task_cycle, space:)
    FactoryBot.build_stubbed_list(:task_event, count, :today_end, :completed, :assigned, assigned_user:, task_cycle:)
  end
end
