class SendSetting < ApplicationRecord
  ...
  validates :slack_webhook_url, format: { with: URL_FORMAT }, allow_blank: true, if: -> { errors[:slack_webhook_url].blank? }
  validates :slack_mention, length: { maximum: Settings.slack_mention_maximum }, allow_blank: true
  validates :slack_mention, format: { with: MENTION_FORMAT }, if: -> { errors[:slack_mention].blank? }
  validates :email_enabled, inclusion: { in: [true, false] } # NOTE: presenceだとfalseもエラーになる為
  validates :email_address, presence: true, if: -> { email_enabled }
